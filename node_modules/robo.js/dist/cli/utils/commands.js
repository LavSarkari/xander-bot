import { ContextMenuCommandBuilder, SlashCommandBuilder, REST, ApplicationIntegrationType, Routes, InteractionContextType } from 'discord.js';
import { Logger } from '../../core/logger.js';
import { Boot } from '../../internal/boot.js';
import { loadConfig } from '../../core/config.js';
import { DEFAULT_CONFIG, FLASHCORE_KEYS, discordLogger } from '../../core/constants.js';
...
function buildContextCommands(dev, contextCommands, type, config) {
  if (dev) {
    logger = new Logger({
      enabled: true,
      level: "info"
  ...
  return Object.entries(contextCommands).map(([key, entry]) => {
    ...
    const commandBuilder = new ContextMenuCommandBuilder().setContexts((entry.contexts ?? defaultContexts).map(getContextType)).setIntegrationTypes((entry.integrationTypes ?? defaultIntegrationTypes).map(getIntegrationType)).setName(key).setNameLocalizations(entry.nameLocalizations || {}).setType(type === "message" ? 3 : 2);
    const defaultMemberPermissions = entry.defaultMemberPermissions ?? config.defaults?.defaultMemberPermissions;
    if (defaultMemberPermissions !== void 0) {
...
function buildSlashCommands(dev, commands, config) {
  if (dev) {
    logger = new Logger({
      enabled: true,
      level: "info"
  ...
  return Object.entries(commands).map(([key, entry]) => {
    ...
    let commandBuilder;
    try {
      commandBuilder = new SlashCommandBuilder().setName(key).setContexts((entry.contexts ?? defaultContexts).map(getContextType)).setIntegrationTypes((entry.integrationTypes ?? defaultIntegrationTypes).map(getIntegrationType)).setNameLocalizations(entry.nameLocalizations || {}).setDescription(entry.description || "No description provided").setDescriptionLocalizations(entry.descriptionLocalizations || {});
    } catch (e) {
...
async function registerCommands(dev, force, newCommands, newMessageContextCommands, newUserContextCommands, changedCommands, addedCommands, removedCommands, changedContextCommands, addedContextCommands, removedContextCommands) {
  ...
  try {
    ...
    let entryCommand = existingCommands.find((command) => command.type === 4);
...
export { addOptionToCommandBuilder, buildContextCommands, buildSlashCommands, findCommandDifferences, getContextType, getIntegrationType, registerCommands };
