...
import { Indent, cloudflareLogger, FLASHCORE_KEYS, HighlightGreen, Highlight, DEFAULT_CONFIG } from '../../core/constants.js';
...
const command = new Command("dev").description("Ready, set, code your bot to life! Starts development mode.").option("-h", "--help", "Shows the available command options").option("-id", "--instance-id", "specify the instance ID to use").option("-l", "--log-level", "specify the log level to use (debug, info, warn, error)").option("-m", "--mode", "specify the mode(s) to run in (dev, beta, prod, etc...)").option("-s", "--silent", "do not print anything").option("-t", "--tunnel", "expose your local server to the internet").option("-v", "--verbose", "print more information for debugging").handler(devAction);
...
async function devAction(_args, options) {
  ...
  logger.log("");
  logger.log(Indent, color.bold(`\u{1F680} Starting ${color.cyan(projectName)} in ${Mode.color(Mode.get())} mode`));
  ...
  const experimentalKeys = Object.entries(config.experimental ?? {}).filter(([, value]) => value).map(([key]) => key).filter((key) => key !== "disableBot");
...
async function checkUpdates(config, forceCheck = false, suggest = true) {
  ...
  if (update.hasUpdate) {
    ...
    let message = `
${Indent} \u{1F4A1} ${HighlightGreen("Update available!")} ${versionDelta}
`;
    if (suggest) {
      const command2 = `${packageExecutor} sage upgrade`;
      message += `${Indent}    Run ${Highlight(command2)} or disable update checks in config.
`;
    }
    logger.log(message);
